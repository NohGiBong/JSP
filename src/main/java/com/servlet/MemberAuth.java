...
/*
        ...
        String oId = application.getInitParameter("OracldId");
        String oPass = application.getInitParameter("OraclePwd");
*/

        // DAO 생성
        dao = new MemberDAO();
        /*driver, connectUrl, oId, oPass*/
    }
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (memberName != null) { // 일치하는 회원 찾음
            req.setAttribute("authMsg", memberName + " 회원님 안녕하세요.");
        } else { // 일치하는 회원 없음
            if (admin_id.equals(id)) // 관리자
                req.setAttribute("authMsg", admin_id + "는 최고 관리자입니다.");
            else // 비회원
            req.setAttribute("authMsg", "회원 정보가 없습니다.");
        }
